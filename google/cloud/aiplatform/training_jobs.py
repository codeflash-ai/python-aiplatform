...
_PIPELINE_COMPLETE_STATES = set([gca_pipeline_state.PipelineState.PIPELINE_STATE_SUCCEEDED, gca_pipeline_state.PipelineState.PIPELINE_STATE_FAILED, gca_pipeline_state.PipelineState.PIPELINE_STATE_CANCELLED, gca_pipeline_state.PipelineState.PIPELINE_STATE_PAUSED])
_JOB_WAIT_TIME = 5
_LOG_WAIT_TIME = 5
_MAX_WAIT_TIME = 60 * 5
...
class _TrainingJob(base.VertexAiStatefulResource):
    ...
    @codeflash_capture(function_name='_TrainingJob.__init__', tmp_dir_path='/tmp/codeflash_isahfmnp/test_return_values', tests_root='/home/ubuntu/work/repo/tests', is_fto=True)
    def __init__(self, display_name: Optional[str]=None, project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None, labels: Optional[Dict[str, str]]=None, training_encryption_spec_key_name: Optional[str]=None, model_encryption_spec_key_name: Optional[str]=None):
        ...
        self._labels = labels
        self._training_encryption_spec = initializer.global_config.get_encryption_spec(encryption_spec_key_name=training_encryption_spec_key_name)
        self._model_encryption_spec = initializer.global_config.get_encryption_spec(encryption_spec_key_name=model_encryption_spec_key_name)
    ...
    @classmethod
    def get(cls, resource_name: str, project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None) -> '_TrainingJob':
        ...
        """
        self = cls._empty_constructor(project=project, location=location, credentials=credentials, resource_name=resource_name)
        self._gca_resource = self._get_gca_resource(resource_name=resource_name)
        if self._gca_resource.training_task_definition not in cls._supported_training_schemas:
            raise ValueError(f'The task definition of the retrieved training job is {self._gca_resource.training_task_definition}, which is not compatible with {cls.__name__}.')
        return self

    @classmethod
    def _get_and_return_subclass(cls, resource_name: str, project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None) -> '_TrainingJob':
        ...
        class_list = [c for c in cls.__subclasses__() if c.__name__.startswith('AutoML')] + [CustomTrainingJob]
        for c in class_list:
            if schema_uri in c._supported_training_schemas:
                return c._empty_constructor(project=project, location=location, credentials=credentials, resource_name=resource_name)
    ...
    def _create_input_data_config(dataset: Optional[datasets._Dataset]=None, annotation_schema_uri: Optional[str]=None, training_fraction_split: Optional[float]=None, validation_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, training_filter_split: Optional[str]=None, validation_filter_split: Optional[str]=None, test_filter_split: Optional[str]=None, predefined_split_column_name: Optional[str]=None, timestamp_split_column_name: Optional[str]=None, gcs_destination_uri_prefix: Optional[str]=None, bigquery_destination: Optional[str]=None) -> Optional[gca_training_pipeline.InputDataConfig]:
        ...
        if dataset:
            ...
            fraction_split = None
            if any([training_filter_split is not None, validation_filter_split is not None, test_filter_split is not None]):
                if all([training_filter_split is not None, validation_filter_split is not None, test_filter_split is not None]):
                    filter_split = gca_training_pipeline.FilterSplit(training_filter=training_filter_split, validation_filter=validation_filter_split, test_filter=test_filter_split)
                else:
                    raise ValueError('All filter splits must be passed together or not at all')
            if predefined_split_column_name:
                predefined_split = gca_training_pipeline.PredefinedSplit(key=predefined_split_column_name)
            if timestamp_split_column_name:
                timestamp_split = gca_training_pipeline.TimestampSplit(training_fraction=training_fraction_split, validation_fraction=validation_fraction_split, test_fraction=test_fraction_split, key=timestamp_split_column_name)
            elif any([training_fraction_split is not None, validation_fraction_split is not None, test_fraction_split is not None]):
                fraction_split = gca_training_pipeline.FractionSplit(training_fraction=training_fraction_split, validation_fraction=validation_fraction_split, test_fraction=test_fraction_split)
            splits = [split for split in [filter_split, predefined_split, timestamp_split_column_name, fraction_split] if split is not None]
            if len(splits) == 0:
                _LOGGER.info('No dataset split provided. The service will use a default split.')
            elif len(splits) > 1:
                raise ValueError("You've specified too many split types. You can specify only one of the following:    1. `training_filter_split`, `validation_filter_split`, `test_filter_split`    2. `predefined_split_column_name`    3. `timestamp_split_column_name`, `training_fraction_split`, `validation_fraction_split`, `test_fraction_split`    4.`training_fraction_split`, `validation_fraction_split`, `test_fraction_split`")
            gcs_destination = None
            if gcs_destination_uri_prefix:
                gcs_destination = gca_io.GcsDestination(output_uri_prefix=gcs_destination_uri_prefix)
            bigquery_destination_proto = None
            if bigquery_destination:
                bigquery_destination_proto = gca_io.BigQueryDestination(output_uri=bigquery_destination)
            input_data_config = gca_training_pipeline.InputDataConfig(fraction_split=fraction_split, filter_split=filter_split, predefined_split=predefined_split, timestamp_split=timestamp_split, dataset_id=dataset.name, annotation_schema_uri=annotation_schema_uri, gcs_destination=gcs_destination, bigquery_destination=bigquery_destination_proto)
        return input_data_config

    def _run_job(self, training_task_definition: str, training_task_inputs: Union[dict, proto.Message], dataset: Optional[datasets._Dataset], training_fraction_split: Optional[float]=None, validation_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, training_filter_split: Optional[str]=None, validation_filter_split: Optional[str]=None, test_filter_split: Optional[str]=None, predefined_split_column_name: Optional[str]=None, timestamp_split_column_name: Optional[str]=None, annotation_schema_uri: Optional[str]=None, model: Optional[gca_model.Model]=None, model_id: Optional[str]=None, parent_model: Optional[str]=None, is_default_version: Optional[bool]=True, model_version_aliases: Optional[Sequence[str]]=None, model_version_description: Optional[str]=None, gcs_destination_uri_prefix: Optional[str]=None, bigquery_destination: Optional[str]=None, create_request_timeout: Optional[float]=None, block: Optional[bool]=True) -> Optional[models.Model]:
        """Runs the training job.
    ...
    def _is_waiting_to_run(self) -> bool:
        ...
        if self._latest_future:
            _LOGGER.info('The training job is waiting for upstream SDK tasks to complete before launching.')
            return True
        return False
    ...
    def _get_model(self, block: bool=True) -> Optional[models.Model]:
        ...
        if self.has_failed:
            raise RuntimeError(f'Training Pipeline {self.resource_name} failed. No model available.')
        if not self._gca_resource.model_to_upload:
            return None
        if self._gca_resource.model_to_upload.name:
            return models.Model(model_name=self._gca_resource.model_to_upload.name, version=self._gca_resource.model_to_upload.version_id)
    ...
    def _block_until_complete(self):
        ...
        while self.state not in _PIPELINE_COMPLETE_STATES:
            ...
            if current_time - previous_time >= log_wait:
                _LOGGER.info('%s %s current state:\n%s' % (self.__class__.__name__, self._gca_resource.name, self._gca_resource.state))
        ...
        if self._gca_resource.model_to_upload and (not self.has_failed):
            _LOGGER.info('Model available at %s' % self._gca_resource.model_to_upload.name)
    ...
    def _assert_has_run(self) -> bool:
        ...
        if not self._has_run:
            if self._is_waiting_to_run():
                return True
            raise RuntimeError('TrainingPipeline has not been launched. You must run this TrainingPipeline using TrainingPipeline.run. ')
        return False

    @classmethod
    def list(cls, filter: Optional[str]=None, order_by: Optional[str]=None, project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None) -> List['base.VertexAiResourceNoun']:
        ...
        """
        training_job_subclass_filter = lambda gapic_obj: gapic_obj.training_task_definition in cls._supported_training_schemas
        return cls._list_with_local_order(cls_filter=training_job_subclass_filter, filter=filter, order_by=order_by, project=project, location=location, credentials=credentials)
    ...
    def cancel(self) -> None:
        ...
        """
        if not self._has_run:
            raise RuntimeError('This TrainingJob has not been launched, use the `run()` method to start. `cancel()` can only be called on a job that is running.')
...
class _CustomTrainingJob(_TrainingJob):
    ...
    def __init__(self, display_name: str, container_uri: str, model_serving_container_image_uri: Optional[str]=None, model_serving_container_predict_route: Optional[str]=None, model_serving_container_health_route: Optional[str]=None, model_serving_container_command: Optional[Sequence[str]]=None, model_serving_container_args: Optional[Sequence[str]]=None, model_serving_container_environment_variables: Optional[Dict[str, str]]=None, model_serving_container_ports: Optional[Sequence[int]]=None, model_description: Optional[str]=None, model_instance_schema_uri: Optional[str]=None, model_parameters_schema_uri: Optional[str]=None, model_prediction_schema_uri: Optional[str]=None, explanation_metadata: Optional[explain.ExplanationMetadata]=None, explanation_parameters: Optional[explain.ExplanationParameters]=None, project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None, labels: Optional[Dict[str, str]]=None, training_encryption_spec_key_name: Optional[str]=None, model_encryption_spec_key_name: Optional[str]=None, staging_bucket: Optional[str]=None):
        ...
        if not display_name:
            display_name = self.__class__._generate_display_name()
        super().__init__(display_name=display_name, project=project, location=location, credentials=credentials, labels=labels, training_encryption_spec_key_name=training_encryption_spec_key_name, model_encryption_spec_key_name=model_encryption_spec_key_name)
        self._container_uri = container_uri
        model_predict_schemata = None
        if any([model_instance_schema_uri, model_parameters_schema_uri, model_prediction_schema_uri]):
            model_predict_schemata = gca_model.PredictSchemata(instance_schema_uri=model_instance_schema_uri, parameters_schema_uri=model_parameters_schema_uri, prediction_schema_uri=model_prediction_schema_uri)
        env = None
        ports = None
        if model_serving_container_environment_variables:
            env = [gca_env_var.EnvVar(name=str(key), value=str(value)) for (key, value) in model_serving_container_environment_variables.items()]
        if model_serving_container_ports:
            ports = [gca_model.Port(container_port=port) for port in model_serving_container_ports]
        container_spec = gca_model.ModelContainerSpec(image_uri=model_serving_container_image_uri, command=model_serving_container_command, args=model_serving_container_args, env=env, ports=ports, predict_route=model_serving_container_predict_route, health_route=model_serving_container_health_route)
        self._managed_model = gca_model.Model(description=model_description, predict_schemata=model_predict_schemata, container_spec=container_spec, encryption_spec=self._model_encryption_spec)
        self._staging_bucket = staging_bucket or initializer.global_config.staging_bucket
        if not self._staging_bucket:
            raise RuntimeError("staging_bucket should be set in TrainingJob constructor or set using aiplatform.init(staging_bucket='gs://my-bucket')")
    ...
    def _prepare_training_task_inputs_and_output_dir(self, worker_pool_specs: worker_spec_utils._DistributedTrainingSpec, base_output_dir: Optional[str]=None, service_account: Optional[str]=None, network: Optional[str]=None, timeout: Optional[int]=None, restart_job_on_worker_restart: bool=False, enable_web_access: bool=False, enable_dashboard_access: bool=False, tensorboard: Optional[str]=None, disable_retries: bool=False, persistent_resource_id: Optional[str]=None, scheduling_strategy: Optional[gca_custom_job_compat.Scheduling.Strategy]=None, max_wait_duration: Optional[int]=None, psc_interface_config: Optional[gca_service_networking.PscInterfaceConfig]=None) -> Tuple[Dict, str]:
        ...
        """
        base_output_dir = base_output_dir or _timestamped_gcs_dir(self._staging_bucket, 'aiplatform-custom-training')
        _LOGGER.info('Training Output directory:\n%s ' % base_output_dir)
        training_task_inputs = {'worker_pool_specs': worker_pool_specs, 'base_output_directory': {'output_uri_prefix': base_output_dir}}
        if service_account:
        ...
        if timeout or restart_job_on_worker_restart or disable_retries or scheduling_strategy or max_wait_duration:
            timeout = f'{timeout}s' if timeout else None
            max_wait_duration = f'{max_wait_duration}s' if max_wait_duration else None
            scheduling = {'timeout': timeout, 'restart_job_on_worker_restart': restart_job_on_worker_restart, 'disable_retries': disable_retries, 'strategy': scheduling_strategy, 'max_wait_duration': max_wait_duration}
    ...
    def web_access_uris(self) -> Dict[str, str]:
        ...
        if self._gca_resource.training_task_metadata and self._gca_resource.training_task_metadata.get('backingCustomJob'):
            custom_job_resource_name = self._gca_resource.training_task_metadata.get('backingCustomJob')
    ...
    def _log_web_access_uris(self):
        ...
        for (worker, uri) in self.web_access_uris.items():
            if uri not in self._logged_web_access_uris:
                _LOGGER.info('%s %s access the interactive shell terminals for the backing custom job:\n%s:\n%s' % (self.__class__.__name__, self._gca_resource.name, worker, uri))
                self._logged_web_access_uris.add(uri)

    def _wait_callback(self):
        if self._gca_resource.training_task_metadata and self._gca_resource.training_task_metadata.get('backingCustomJob') and (not self._has_logged_custom_job):
        ...
        if self._gca_resource.training_task_inputs.get('enable_web_access') or self._gca_resource.training_task_inputs.get('enable_dashboard_access'):
    ...
    def _tensorboard_console_uri(self) -> str:
        ...
        tensorboard_resource_name = self._gca_resource.training_task_inputs.get('tensorboard')
        custom_job_resource_name = self._gca_resource.training_task_metadata.get('backingCustomJob')
    ...
    def _model_upload_fail_string(self) -> str:
        """Helper property for model upload failure."""
        return f"Training Pipeline {self.resource_name} is not configured to upload a Model. Create the Training Pipeline with model_serving_container_image_uri and model_display_name passed in. Ensure that your training script saves to model to os.environ['AIP_MODEL_DIR']."
...
class _ForecastingTrainingJob(_TrainingJob):
    ...
    def __init__(self, display_name: Optional[str]=None, optimization_objective: Optional[str]=None, column_specs: Optional[Dict[str, str]]=None, column_transformations: Optional[List[Dict[str, Dict[str, str]]]]=None, project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None, labels: Optional[Dict[str, str]]=None, training_encryption_spec_key_name: Optional[str]=None, model_encryption_spec_key_name: Optional[str]=None):
        ...
        """
        super().__init__(display_name=display_name, project=project, location=location, credentials=credentials, labels=labels, training_encryption_spec_key_name=training_encryption_spec_key_name, model_encryption_spec_key_name=model_encryption_spec_key_name)
        self._column_transformations = column_transformations_utils.validate_and_get_column_transformations(column_specs, column_transformations)
    ...
    def _run(self, dataset: datasets.TimeSeriesDataset, target_column: str, time_column: str, time_series_identifier_column: str, unavailable_at_forecast_columns: List[str], available_at_forecast_columns: List[str], forecast_horizon: int, data_granularity_unit: str, data_granularity_count: int, training_fraction_split: Optional[float]=None, validation_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, predefined_split_column_name: Optional[str]=None, timestamp_split_column_name: Optional[str]=None, weight_column: Optional[str]=None, time_series_attribute_columns: Optional[List[str]]=None, context_window: Optional[int]=None, export_evaluated_data_items: bool=False, export_evaluated_data_items_bigquery_destination_uri: Optional[str]=None, export_evaluated_data_items_override_destination: bool=False, quantiles: Optional[List[float]]=None, validation_options: Optional[str]=None, budget_milli_node_hours: int=1000, model_display_name: Optional[str]=None, model_labels: Optional[Dict[str, str]]=None, model_id: Optional[str]=None, parent_model: Optional[str]=None, is_default_version: Optional[bool]=True, model_version_aliases: Optional[Sequence[str]]=None, model_version_description: Optional[str]=None, hierarchy_group_columns: Optional[List[str]]=None, hierarchy_group_total_weight: Optional[float]=None, hierarchy_temporal_total_weight: Optional[float]=None, hierarchy_group_temporal_total_weight: Optional[float]=None, window_column: Optional[str]=None, window_stride_length: Optional[int]=None, window_max_count: Optional[int]=None, holiday_regions: Optional[List[str]]=None, sync: bool=True, create_request_timeout: Optional[float]=None, enable_probabilistic_inference: bool=False) -> models.Model:
        ...
        if self._column_transformations is None:
            _LOGGER.info('No column transformations provided, so now retrieving columns from dataset in order to set default column transformations.')
            (self._column_transformations, column_names) = column_transformations_utils.get_default_column_transformations(dataset=dataset, target_column=target_column)
            _LOGGER.info("The column transformation of type 'auto' was set for the following columns: %s." % column_names)
        window_config = self._create_window_config(column=window_column, stride_length=window_stride_length, max_count=window_max_count)
        additional_experiment_probabilistic_inference = self._convert_enable_probabilistic_inference()
        if not enable_probabilistic_inference:
            enable_probabilistic_inference = additional_experiment_probabilistic_inference
        training_task_inputs_dict = {'targetColumn': target_column, 'timeColumn': time_column, 'timeSeriesIdentifierColumn': time_series_identifier_column, 'timeSeriesAttributeColumns': time_series_attribute_columns, 'unavailableAtForecastColumns': unavailable_at_forecast_columns, 'availableAtForecastColumns': available_at_forecast_columns, 'forecastHorizon': forecast_horizon, 'dataGranularity': {'unit': data_granularity_unit, 'quantity': data_granularity_count}, 'transformations': self._column_transformations, 'trainBudgetMilliNodeHours': budget_milli_node_hours, 'weightColumn': weight_column, 'contextWindow': context_window, 'quantiles': quantiles, 'validationOptions': validation_options, 'optimizationObjective': self._optimization_objective, 'holidayRegions': holiday_regions}
        if any([hierarchy_group_columns, hierarchy_group_total_weight, hierarchy_temporal_total_weight, hierarchy_group_temporal_total_weight]):
            training_task_inputs_dict['hierarchyConfig'] = {'groupColumns': hierarchy_group_columns, 'groupTotalWeight': hierarchy_group_total_weight, 'temporalTotalWeight': hierarchy_temporal_total_weight, 'groupTemporalTotalWeight': hierarchy_group_temporal_total_weight}
        if window_config:
            training_task_inputs_dict['windowConfig'] = window_config
        if enable_probabilistic_inference:
            training_task_inputs_dict['enableProbabilisticInference'] = enable_probabilistic_inference
        ...
        if export_evaluated_data_items:
            training_task_inputs_dict['exportEvaluatedDataItemsConfig'] = {'destinationBigqueryUri': final_export_eval_bq_uri, 'overrideExistingTable': export_evaluated_data_items_override_destination}
        if self._additional_experiments:
            training_task_inputs_dict['additionalExperiments'] = self._additional_experiments
        model = gca_model.Model(display_name=model_display_name or self._display_name, labels=model_labels or self._labels, encryption_spec=self._model_encryption_spec)
        new_model = self._run_job(training_task_definition=self._training_task_definition, training_task_inputs=training_task_inputs_dict, dataset=dataset, training_fraction_split=training_fraction_split, validation_fraction_split=validation_fraction_split, test_fraction_split=test_fraction_split, predefined_split_column_name=predefined_split_column_name, timestamp_split_column_name=timestamp_split_column_name, model=model, model_id=model_id, parent_model=parent_model, is_default_version=is_default_version, model_version_aliases=model_version_aliases, model_version_description=model_version_description, create_request_timeout=create_request_timeout)
        if export_evaluated_data_items:
    ...
    def _model_upload_fail_string(self) -> str:
        """Helper property for model upload failure."""
        return f'Training Pipeline {self.resource_name} is not configured to upload a Model.'
    ...
    @staticmethod
    def _create_window_config(column: Optional[str]=None, stride_length: Optional[int]=None, max_count: Optional[int]=None) -> Optional[Dict[str, Union[int, str]]]:
        """Creates a window config from training job arguments."""
        if column is not None:
            if stride_length is not None or max_count is not None:
        ...
        if stride_length is not None:
            if max_count is not None:
                raise ValueError('More than one windowing strategy provided. Make sure only one of window_column, window_stride_length, or window_max_count is specified.')
            return {'strideLength': stride_length}
        if max_count is not None:
            return {'maxCount': max_count}
        return None
...
class CustomTrainingJob(_CustomTrainingJob):
    ...
    def submit(self, dataset: Optional[Union[datasets.ImageDataset, datasets.TabularDataset, datasets.TextDataset, datasets.VideoDataset]]=None, annotation_schema_uri: Optional[str]=None, model_display_name: Optional[str]=None, model_labels: Optional[Dict[str, str]]=None, model_id: Optional[str]=None, parent_model: Optional[str]=None, is_default_version: Optional[bool]=True, model_version_aliases: Optional[Sequence[str]]=None, model_version_description: Optional[str]=None, base_output_dir: Optional[str]=None, service_account: Optional[str]=None, network: Optional[str]=None, bigquery_destination: Optional[str]=None, args: Optional[List[Union[str, float, int]]]=None, environment_variables: Optional[Dict[str, str]]=None, replica_count: int=1, machine_type: str='n1-standard-4', accelerator_type: str='ACCELERATOR_TYPE_UNSPECIFIED', accelerator_count: int=0, boot_disk_type: str='pd-ssd', boot_disk_size_gb: int=100, reduction_server_replica_count: int=0, reduction_server_machine_type: Optional[str]=None, reduction_server_container_uri: Optional[str]=None, training_fraction_split: Optional[float]=None, validation_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, training_filter_split: Optional[str]=None, validation_filter_split: Optional[str]=None, test_filter_split: Optional[str]=None, predefined_split_column_name: Optional[str]=None, timestamp_split_column_name: Optional[str]=None, timeout: Optional[int]=None, restart_job_on_worker_restart: bool=False, enable_web_access: bool=False, enable_dashboard_access: bool=False, tensorboard: Optional[str]=None, sync=True, create_request_timeout: Optional[float]=None, disable_retries: bool=False, persistent_resource_id: Optional[str]=None, tpu_topology: Optional[str]=None, scheduling_strategy: Optional[gca_custom_job_compat.Scheduling.Strategy]=None, reservation_affinity_type: Optional[Literal['NO_RESERVATION', 'ANY_RESERVATION', 'SPECIFIC_RESERVATION']]=None, reservation_affinity_key: Optional[str]=None, reservation_affinity_values: Optional[List[str]]=None, max_wait_duration: Optional[int]=None, psc_interface_config: Optional[gca_service_networking.PscInterfaceConfig]=None) -> Optional[models.Model]:
    ...
    def _run(self, python_packager: source_utils._TrainingScriptPythonPackager, dataset: Optional[Union[datasets.ImageDataset, datasets.TabularDataset, datasets.TextDataset, datasets.VideoDataset]], annotation_schema_uri: Optional[str], worker_pool_specs: worker_spec_utils._DistributedTrainingSpec, managed_model: Optional[gca_model.Model]=None, model_id: Optional[str]=None, parent_model: Optional[str]=None, is_default_version: Optional[bool]=True, model_version_aliases: Optional[Sequence[str]]=None, model_version_description: Optional[str]=None, args: Optional[List[Union[str, float, int]]]=None, environment_variables: Optional[Dict[str, str]]=None, base_output_dir: Optional[str]=None, service_account: Optional[str]=None, network: Optional[str]=None, bigquery_destination: Optional[str]=None, training_fraction_split: Optional[float]=None, validation_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, training_filter_split: Optional[str]=None, validation_filter_split: Optional[str]=None, test_filter_split: Optional[str]=None, predefined_split_column_name: Optional[str]=None, timestamp_split_column_name: Optional[str]=None, timeout: Optional[int]=None, restart_job_on_worker_restart: bool=False, enable_web_access: bool=False, enable_dashboard_access: bool=False, tensorboard: Optional[str]=None, reduction_server_container_uri: Optional[str]=None, sync=True, create_request_timeout: Optional[float]=None, block: Optional[bool]=True, disable_retries: bool=False, persistent_resource_id: Optional[str]=None, scheduling_strategy: Optional[gca_custom_job_compat.Scheduling.Strategy]=None, max_wait_duration: Optional[int]=None, psc_interface_config: Optional[gca_service_networking.PscInterfaceConfig]=None) -> Optional[models.Model]:
        ...
        Returns:
            model: The trained Vertex AI Model resource or None if training did not
                produce a Vertex AI Model.
        """
        package_gcs_uri = python_packager.package_and_copy_to_gcs(gcs_staging_dir=self._staging_bucket, project=self.project, credentials=self.credentials)
        for (spec_order, spec) in enumerate(worker_pool_specs):
            if not spec:
                continue
            if spec_order == worker_spec_utils._SPEC_ORDERS['server_spec'] and reduction_server_container_uri:
                spec['container_spec'] = {'image_uri': reduction_server_container_uri}
            else:
                spec['python_package_spec'] = {'executor_image_uri': self._container_uri, 'python_module': python_packager.module_name, 'package_uris': [package_gcs_uri]}
                if args:
                    spec['python_package_spec']['args'] = args
                if environment_variables:
                    spec['python_package_spec']['env'] = [{'name': key, 'value': value} for (key, value) in environment_variables.items()]
...
class CustomContainerTrainingJob(_CustomTrainingJob):
    ...
    def __init__(self, display_name: str, container_uri: str, command: Optional[Sequence[str]]=None, model_serving_container_image_uri: Optional[str]=None, model_serving_container_predict_route: Optional[str]=None, model_serving_container_health_route: Optional[str]=None, model_serving_container_command: Optional[Sequence[str]]=None, model_serving_container_args: Optional[Sequence[str]]=None, model_serving_container_environment_variables: Optional[Dict[str, str]]=None, model_serving_container_ports: Optional[Sequence[int]]=None, model_description: Optional[str]=None, model_instance_schema_uri: Optional[str]=None, model_parameters_schema_uri: Optional[str]=None, model_prediction_schema_uri: Optional[str]=None, explanation_metadata: Optional[explain.ExplanationMetadata]=None, explanation_parameters: Optional[explain.ExplanationParameters]=None, project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None, labels: Optional[Dict[str, str]]=None, training_encryption_spec_key_name: Optional[str]=None, model_encryption_spec_key_name: Optional[str]=None, staging_bucket: Optional[str]=None):
    ...
    @base.optional_sync(construct_object_on_arg='managed_model')
    def _run(self, dataset: Optional[Union[datasets.ImageDataset, datasets.TabularDataset, datasets.TextDataset, datasets.VideoDataset]], annotation_schema_uri: Optional[str], worker_pool_specs: worker_spec_utils._DistributedTrainingSpec, managed_model: Optional[gca_model.Model]=None, model_id: Optional[str]=None, parent_model: Optional[str]=None, is_default_version: Optional[bool]=True, model_version_aliases: Optional[Sequence[str]]=None, model_version_description: Optional[str]=None, args: Optional[List[Union[str, float, int]]]=None, environment_variables: Optional[Dict[str, str]]=None, base_output_dir: Optional[str]=None, service_account: Optional[str]=None, network: Optional[str]=None, bigquery_destination: Optional[str]=None, training_fraction_split: Optional[float]=None, validation_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, training_filter_split: Optional[str]=None, validation_filter_split: Optional[str]=None, test_filter_split: Optional[str]=None, predefined_split_column_name: Optional[str]=None, timestamp_split_column_name: Optional[str]=None, timeout: Optional[int]=None, restart_job_on_worker_restart: bool=False, enable_web_access: bool=False, enable_dashboard_access: bool=False, tensorboard: Optional[str]=None, reduction_server_container_uri: Optional[str]=None, sync=True, create_request_timeout: Optional[float]=None, block: Optional[bool]=True, disable_retries: bool=False, persistent_resource_id: Optional[str]=None, scheduling_strategy: Optional[gca_custom_job_compat.Scheduling.Strategy]=None, max_wait_duration: Optional[int]=None, psc_interface_config: Optional[gca_service_networking.PscInterfaceConfig]=None) -> Optional[models.Model]:
        """Packages local script and launches training_job.
        Args:
            dataset (
                Union[
        ...
        for (spec_order, spec) in enumerate(worker_pool_specs):
            ...
            if spec_order == worker_spec_utils._SPEC_ORDERS['server_spec'] and reduction_server_container_uri:
                ...
                if environment_variables:
                    spec['containerSpec']['env'] = [{'name': key, 'value': value} for (key, value) in environment_variables.items()]
        (training_task_inputs, base_output_dir) = self._prepare_training_task_inputs_and_output_dir(worker_pool_specs=worker_pool_specs, base_output_dir=base_output_dir, service_account=service_account, network=network, timeout=timeout, restart_job_on_worker_restart=restart_job_on_worker_restart, enable_web_access=enable_web_access, enable_dashboard_access=enable_dashboard_access, tensorboard=tensorboard, disable_retries=disable_retries, persistent_resource_id=persistent_resource_id, scheduling_strategy=scheduling_strategy, max_wait_duration=max_wait_duration, psc_interface_config=psc_interface_config)
        model = self._run_job(training_task_definition=schema.training_job.definition.custom_task, training_task_inputs=training_task_inputs, dataset=dataset, annotation_schema_uri=annotation_schema_uri, training_fraction_split=training_fraction_split, validation_fraction_split=validation_fraction_split, test_fraction_split=test_fraction_split, training_filter_split=training_filter_split, validation_filter_split=validation_filter_split, test_filter_split=test_filter_split, predefined_split_column_name=predefined_split_column_name, timestamp_split_column_name=timestamp_split_column_name, model=managed_model, model_id=model_id, parent_model=parent_model, is_default_version=is_default_version, model_version_aliases=model_version_aliases, model_version_description=model_version_description, gcs_destination_uri_prefix=base_output_dir, bigquery_destination=bigquery_destination, create_request_timeout=create_request_timeout, block=block)
        return model
...
class AutoMLTabularTrainingJob(_TrainingJob):
    ...
    def _run(self, dataset: datasets.TabularDataset, target_column: str, training_fraction_split: Optional[float]=None, validation_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, predefined_split_column_name: Optional[str]=None, timestamp_split_column_name: Optional[str]=None, weight_column: Optional[str]=None, budget_milli_node_hours: int=1000, model_display_name: Optional[str]=None, model_labels: Optional[Dict[str, str]]=None, model_id: Optional[str]=None, parent_model: Optional[str]=None, is_default_version: Optional[bool]=True, model_version_aliases: Optional[Sequence[str]]=None, model_version_description: Optional[str]=None, disable_early_stopping: bool=False, export_evaluated_data_items: bool=False, export_evaluated_data_items_bigquery_destination_uri: Optional[str]=None, export_evaluated_data_items_override_destination: bool=False, sync: bool=True, create_request_timeout: Optional[float]=None) -> models.Model:
        ...
        if self._column_transformations is None:
            ...
            (self._column_transformations, column_names) = column_transformations_utils.get_default_column_transformations(dataset=dataset, target_column=target_column)
            _LOGGER.info("The column transformation of type 'auto' was set for the following columns: %s." % column_names)
        training_task_inputs_dict = {'targetColumn': target_column, 'transformations': self._column_transformations, 'trainBudgetMilliNodeHours': budget_milli_node_hours, 'weightColumnName': weight_column, 'disableEarlyStopping': disable_early_stopping, 'optimizationObjective': self._optimization_objective, 'predictionType': self._optimization_prediction_type, 'optimizationObjectiveRecallValue': self._optimization_objective_recall_value, 'optimizationObjectivePrecisionValue': self._optimization_objective_precision_value}
        final_export_eval_bq_uri = export_evaluated_data_items_bigquery_destination_uri
        if final_export_eval_bq_uri and (not final_export_eval_bq_uri.startswith('bq://')):
            final_export_eval_bq_uri = f'bq://{final_export_eval_bq_uri}'
        if export_evaluated_data_items:
    ...
    @staticmethod
    def get_auto_column_specs(dataset: datasets.TabularDataset, target_column: str) -> Dict[str, str]:
        ...
        """
        column_names = [column for column in dataset.column_names if column != target_column]
...
class SequenceToSequencePlusForecastingTrainingJob(_ForecastingTrainingJob):
    ...
    _supported_training_schemas = (schema.training_job.definition.seq2seq_plus_forecasting,)
...
class AutoMLImageTrainingJob(_TrainingJob):
    _supported_training_schemas = (schema.training_job.definition.automl_image_classification, schema.training_job.definition.automl_image_object_detection)

    def __init__(self, display_name: Optional[str]=None, prediction_type: str='classification', multi_label: bool=False, model_type: str='CLOUD', base_model: Optional[models.Model]=None, incremental_train_base_model: Optional[models.Model]=None, project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None, labels: Optional[Dict[str, str]]=None, training_encryption_spec_key_name: Optional[str]=None, model_encryption_spec_key_name: Optional[str]=None, checkpoint_name: Optional[str]=None, trainer_config: Optional[Dict[str, str]]=None, metric_spec: Optional[Dict[str, str]]=None, parameter_spec: Optional[Dict[str, Union[hpt.DoubleParameterSpec, hpt.IntegerParameterSpec, hpt.CategoricalParameterSpec, hpt.DiscreteParameterSpec]]]=None, search_algorithm: Optional[str]=None, measurement_selection: Optional[str]=None):
        ...
        if not valid_model_types:
            raise ValueError(f"'{prediction_type}' is not a supported prediction type for AutoML Image Training. Please choose one of: {tuple(constants.AUTOML_IMAGE_PREDICTION_MODEL_TYPES.keys())}.")
        if model_type == 'CLOUD' and prediction_type == 'object_detection':
            model_type = 'CLOUD_HIGH_ACCURACY_1'
        if model_type not in valid_model_types:
            raise ValueError(f"'{model_type}' is not a supported model_type for prediction_type of '{prediction_type}'. Please choose one of: {tuple(valid_model_types)}")
        if base_model and prediction_type != 'classification':
            raise ValueError(f"Training with a `base_model` is only supported in AutoML Image Classification. However '{prediction_type}' was provided as `prediction_type`.")
        model_garden_models = constants.MODEL_GARDEN_ICN_MODEL_TYPES.union(constants.MODEL_GARDEN_IOD_MODEL_TYPES)
        if checkpoint_name and model_type not in model_garden_models:
            raise ValueError(f"Training with a `checkpoint_name` is only supported in Model Garden models {tuple(model_garden_models)}. However, '{model_type}' was provided as `model_type`.")
        if trainer_config and model_type not in model_garden_models:
            raise ValueError(f"Training with a `trainer_config` is only supported in Model Garden models {tuple(model_garden_models)}. However, '{model_type}' was provided as `model_type`.")
        if metric_spec and model_type not in model_garden_models:
            raise ValueError(f"Training with a `metric_spec` is only supported in Model Garden models {tuple(model_garden_models)}. However, '{model_type}' was provided as `model_type`.")
        if parameter_spec and model_type not in model_garden_models:
            raise ValueError(f"Training with a `parameter_spec` is only supported in Model Garden models {tuple(model_garden_models)}. However, '{model_type}' was provided as `model_type`.")
        if search_algorithm and model_type not in model_garden_models:
            raise ValueError(f"Training with a `search_algorithm` is only supported in Model Garden models {tuple(model_garden_models)}. However, '{model_type}' was provided as `model_type`.")
        if measurement_selection and model_type not in model_garden_models:
            raise ValueError(f"Training with a `measurement_selection` is only supported in Model Garden models {tuple(model_garden_models)}. However, '{model_type}' was provided as `model_type`.")
        metrics = [gca_study_compat.StudySpec.MetricSpec(metric_id=metric_id, goal=goal.upper()) for (metric_id, goal) in metric_spec.items()] if metric_spec else []
        parameters = [parameter._to_parameter_spec(parameter_id=parameter_id) for (parameter_id, parameter) in parameter_spec.items()] if parameter_spec else []
        study_spec = gca_study_compat.StudySpec(metrics=metrics, parameters=parameters, algorithm=hpt.SEARCH_ALGORITHM_TO_PROTO_VALUE[search_algorithm], measurement_selection_type=hpt.MEASUREMENT_SELECTION_TO_PROTO_VALUE[measurement_selection])
        super().__init__(display_name=display_name, project=project, location=location, credentials=credentials, labels=labels, training_encryption_spec_key_name=training_encryption_spec_key_name, model_encryption_spec_key_name=model_encryption_spec_key_name)
    ...
    def run(self, dataset: datasets.ImageDataset, training_fraction_split: Optional[float]=None, validation_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, training_filter_split: Optional[str]=None, validation_filter_split: Optional[str]=None, test_filter_split: Optional[str]=None, budget_milli_node_hours: Optional[int]=None, model_display_name: Optional[str]=None, model_labels: Optional[Dict[str, str]]=None, model_id: Optional[str]=None, parent_model: Optional[str]=None, is_default_version: Optional[bool]=True, model_version_aliases: Optional[Sequence[str]]=None, model_version_description: Optional[str]=None, disable_early_stopping: bool=False, sync: bool=True, create_request_timeout: Optional[float]=None) -> models.Model:
        ...
        return self._run(dataset=dataset, base_model=self._base_model, incremental_train_base_model=self._incremental_train_base_model, training_fraction_split=training_fraction_split, validation_fraction_split=validation_fraction_split, test_fraction_split=test_fraction_split, training_filter_split=training_filter_split, validation_filter_split=validation_filter_split, test_filter_split=test_filter_split, budget_milli_node_hours=budget_milli_node_hours, model_display_name=model_display_name, model_labels=model_labels, model_id=model_id, parent_model=parent_model, is_default_version=is_default_version, model_version_aliases=model_version_aliases, model_version_description=model_version_description, disable_early_stopping=disable_early_stopping, sync=sync, create_request_timeout=create_request_timeout)

    @base.optional_sync()
    def _run(self, dataset: datasets.ImageDataset, base_model: Optional[models.Model]=None, incremental_train_base_model: Optional[models.Model]=None, training_fraction_split: Optional[float]=None, validation_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, training_filter_split: Optional[str]=None, validation_filter_split: Optional[str]=None, test_filter_split: Optional[str]=None, budget_milli_node_hours: int=1000, model_display_name: Optional[str]=None, model_labels: Optional[Dict[str, str]]=None, model_id: Optional[str]=None, parent_model: Optional[str]=None, is_default_version: Optional[bool]=True, model_version_aliases: Optional[Sequence[str]]=None, model_version_description: Optional[str]=None, disable_early_stopping: bool=False, sync: bool=True, create_request_timeout: Optional[float]=None) -> models.Model:
        """Runs the training job and returns a model.
        ...
        Returns:
            model: The trained Vertex AI Model resource or None if training did not
                produce a Vertex AI Model.
        """
        training_task_definition = getattr(schema.training_job.definition, f'automl_image_{self._prediction_type}')
        training_task_inputs_dict = {'modelType': self._model_type, 'budgetMilliNodeHours': budget_milli_node_hours, 'disableEarlyStopping': disable_early_stopping}
        ...
        if incremental_train_base_model:
            training_task_inputs_dict['uptrainBaseModelId'] = incremental_train_base_model.name
        tunable_parameter_dict: Dict[str, any] = {}
        if self._checkpoint_name:
            tunable_parameter_dict['checkpointName'] = self._checkpoint_name
        if self._study_spec:
            tunable_parameter_dict['studySpec'] = json_format.MessageToDict(self._study_spec._pb)
        ...
        return self._run_job(training_task_definition=training_task_definition, training_task_inputs=training_task_inputs_dict, dataset=dataset, training_fraction_split=training_fraction_split, validation_fraction_split=validation_fraction_split, test_fraction_split=test_fraction_split, training_filter_split=training_filter_split, validation_filter_split=validation_filter_split, test_filter_split=test_filter_split, model=model_tbt, model_id=model_id, parent_model=parent_model, is_default_version=is_default_version, model_version_aliases=model_version_aliases, model_version_description=model_version_description, create_request_timeout=create_request_timeout)

    @property
    def _model_upload_fail_string(self) -> str:
        """Helper property for model upload failure."""
        return f'AutoML Image Training Pipeline {self.resource_name} is not configured to upload a Model.'
...
class CustomPythonPackageTrainingJob(_CustomTrainingJob):
    ...
    def __init__(self, display_name: str, python_package_gcs_uri: Union[str, List[str]], python_module_name: str, container_uri: str, model_serving_container_image_uri: Optional[str]=None, model_serving_container_predict_route: Optional[str]=None, model_serving_container_health_route: Optional[str]=None, model_serving_container_command: Optional[Sequence[str]]=None, model_serving_container_args: Optional[Sequence[str]]=None, model_serving_container_environment_variables: Optional[Dict[str, str]]=None, model_serving_container_ports: Optional[Sequence[int]]=None, model_description: Optional[str]=None, model_instance_schema_uri: Optional[str]=None, model_parameters_schema_uri: Optional[str]=None, model_prediction_schema_uri: Optional[str]=None, explanation_metadata: Optional[explain.ExplanationMetadata]=None, explanation_parameters: Optional[explain.ExplanationParameters]=None, project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None, labels: Optional[Dict[str, str]]=None, training_encryption_spec_key_name: Optional[str]=None, model_encryption_spec_key_name: Optional[str]=None, staging_bucket: Optional[str]=None):
    ...
    @base.optional_sync(construct_object_on_arg='managed_model')
    def _run(self, dataset: Optional[Union[datasets.ImageDataset, datasets.TabularDataset, datasets.TextDataset, datasets.VideoDataset]], annotation_schema_uri: Optional[str], worker_pool_specs: worker_spec_utils._DistributedTrainingSpec, managed_model: Optional[gca_model.Model]=None, model_id: Optional[str]=None, parent_model: Optional[str]=None, is_default_version: Optional[bool]=True, model_version_aliases: Optional[Sequence[str]]=None, model_version_description: Optional[str]=None, args: Optional[List[Union[str, float, int]]]=None, environment_variables: Optional[Dict[str, str]]=None, base_output_dir: Optional[str]=None, service_account: Optional[str]=None, network: Optional[str]=None, training_fraction_split: Optional[float]=None, validation_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, training_filter_split: Optional[str]=None, validation_filter_split: Optional[str]=None, test_filter_split: Optional[str]=None, predefined_split_column_name: Optional[str]=None, timestamp_split_column_name: Optional[str]=None, bigquery_destination: Optional[str]=None, timeout: Optional[int]=None, restart_job_on_worker_restart: bool=False, enable_web_access: bool=False, enable_dashboard_access: bool=False, tensorboard: Optional[str]=None, reduction_server_container_uri: Optional[str]=None, sync=True, create_request_timeout: Optional[float]=None, disable_retries: bool=False, persistent_resource_id: Optional[str]=None, scheduling_strategy: Optional[gca_custom_job_compat.Scheduling.Strategy]=None, max_wait_duration: Optional[int]=None, psc_interface_config: Optional[gca_service_networking.PscInterfaceConfig]=None) -> Optional[models.Model]:
        """Packages local script and launches training_job.
        ...
        for (spec_order, spec) in enumerate(worker_pool_specs):
            ...
            if spec_order == worker_spec_utils._SPEC_ORDERS['server_spec'] and reduction_server_container_uri:
                ...
                spec['python_package_spec'] = {'executor_image_uri': self._container_uri, 'python_module': self._python_module, 'package_uris': self._package_gcs_uri}
                if args:
                    spec['python_package_spec']['args'] = args
                if environment_variables:
                    spec['python_package_spec']['env'] = [{'name': key, 'value': value} for (key, value) in environment_variables.items()]
...
class AutoMLVideoTrainingJob(_TrainingJob):
    _supported_training_schemas = (schema.training_job.definition.automl_video_classification, schema.training_job.definition.automl_video_object_tracking, schema.training_job.definition.automl_video_action_recognition)

    def __init__(self, display_name: Optional[str]=None, prediction_type: str='classification', model_type: str='CLOUD', project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None, labels: Optional[Dict[str, str]]=None, training_encryption_spec_key_name: Optional[str]=None, model_encryption_spec_key_name: Optional[str]=None):
        ...
        if not valid_model_types:
            raise ValueError(f"'{prediction_type}' is not a supported prediction type for AutoML Video Training. Please choose one of: {tuple(constants.AUTOML_VIDEO_PREDICTION_MODEL_TYPES.keys())}.")
        if model_type not in valid_model_types:
            raise ValueError(f"'{model_type}' is not a supported model_type for prediction_type of '{prediction_type}'. Please choose one of: {tuple(valid_model_types)}")
        super().__init__(display_name=display_name, project=project, location=location, credentials=credentials, labels=labels, training_encryption_spec_key_name=training_encryption_spec_key_name, model_encryption_spec_key_name=model_encryption_spec_key_name)
        self._model_type = model_type
    ...
    def run(self, dataset: datasets.VideoDataset, training_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, training_filter_split: Optional[str]=None, test_filter_split: Optional[str]=None, model_display_name: Optional[str]=None, model_labels: Optional[Dict[str, str]]=None, model_id: Optional[str]=None, parent_model: Optional[str]=None, is_default_version: Optional[bool]=True, model_version_aliases: Optional[Sequence[str]]=None, model_version_description: Optional[str]=None, sync: bool=True, create_request_timeout: Optional[float]=None) -> models.Model:
        ...
        return self._run(dataset=dataset, training_fraction_split=training_fraction_split, test_fraction_split=test_fraction_split, training_filter_split=training_filter_split, test_filter_split=test_filter_split, model_display_name=model_display_name, model_labels=model_labels, model_id=model_id, parent_model=parent_model, is_default_version=is_default_version, model_version_aliases=model_version_aliases, model_version_description=model_version_description, sync=sync, create_request_timeout=create_request_timeout)

    @base.optional_sync()
    def _run(self, dataset: datasets.VideoDataset, training_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, training_filter_split: Optional[str]=None, test_filter_split: Optional[str]=None, model_display_name: Optional[str]=None, model_labels: Optional[Dict[str, str]]=None, model_id: Optional[str]=None, parent_model: Optional[str]=None, is_default_version: Optional[bool]=True, model_version_aliases: Optional[Sequence[str]]=None, model_version_description: Optional[str]=None, sync: bool=True, create_request_timeout: Optional[float]=None) -> models.Model:
        """Runs the training job and returns a model.
        ...
        Returns:
            model: The trained Vertex AI Model resource or None if training did not
                produce a Vertex AI Model.
        """
        training_task_definition = getattr(schema.training_job.definition, f'automl_video_{self._prediction_type}')
        training_task_inputs_dict = {'modelType': self._model_type}
        model_tbt = gca_model.Model(encryption_spec=self._model_encryption_spec)
        model_tbt.display_name = model_display_name or self._display_name
        model_tbt.labels = model_labels or self._labels
        validation_filter_split = '-' if all([training_filter_split is not None, test_filter_split is not None]) else None
        return self._run_job(training_task_definition=training_task_definition, training_task_inputs=training_task_inputs_dict, dataset=dataset, training_fraction_split=training_fraction_split, test_fraction_split=test_fraction_split, training_filter_split=training_filter_split, validation_filter_split=validation_filter_split, test_filter_split=test_filter_split, model=model_tbt, model_id=model_id, parent_model=parent_model, is_default_version=is_default_version, model_version_aliases=model_version_aliases, model_version_description=model_version_description, create_request_timeout=create_request_timeout)

    @property
    def _model_upload_fail_string(self) -> str:
        """Helper property for model upload failure."""
        return f'AutoML Video Training Pipeline {self.resource_name} is not configured to upload a Model.'

class AutoMLTextTrainingJob(_TrainingJob):
    _supported_training_schemas = (schema.training_job.definition.automl_text_classification, schema.training_job.definition.automl_text_extraction, schema.training_job.definition.automl_text_sentiment)

    def __init__(self, display_name: str, prediction_type: str, multi_label: bool=False, sentiment_max: int=10, project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None, labels: Optional[Dict[str, str]]=None, training_encryption_spec_key_name: Optional[str]=None, model_encryption_spec_key_name: Optional[str]=None):
        ...
        if prediction_type == 'classification':
            ...
            training_task_inputs_dict = training_job_inputs.AutoMlTextClassificationInputs(multi_label=multi_label)
            ...
            training_task_inputs_dict = training_job_inputs.AutoMlTextSentimentInputs(sentiment_max=sentiment_max)
        else:
            raise ValueError("Prediction type must be one of 'classification', 'extraction', or 'sentiment'.")
        self._training_task_definition = training_task_definition
        self._training_task_inputs_dict = training_task_inputs_dict

    def run(self, dataset: datasets.TextDataset, training_fraction_split: Optional[float]=None, validation_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, training_filter_split: Optional[str]=None, validation_filter_split: Optional[str]=None, test_filter_split: Optional[str]=None, model_display_name: Optional[str]=None, model_labels: Optional[Dict[str, str]]=None, model_id: Optional[str]=None, parent_model: Optional[str]=None, is_default_version: Optional[bool]=True, model_version_aliases: Optional[Sequence[str]]=None, model_version_description: Optional[str]=None, sync: bool=True, create_request_timeout: Optional[float]=None) -> models.Model:
        """Runs the training job and returns a model.
        ...
        return self._run(dataset=dataset, training_fraction_split=training_fraction_split, validation_fraction_split=validation_fraction_split, test_fraction_split=test_fraction_split, training_filter_split=training_filter_split, validation_filter_split=validation_filter_split, test_filter_split=test_filter_split, model_display_name=model_display_name, model_labels=model_labels, model_id=model_id, parent_model=parent_model, is_default_version=is_default_version, model_version_aliases=model_version_aliases, model_version_description=model_version_description, sync=sync, create_request_timeout=create_request_timeout)

    @base.optional_sync()
    def _run(self, dataset: datasets.TextDataset, training_fraction_split: Optional[float]=None, validation_fraction_split: Optional[float]=None, test_fraction_split: Optional[float]=None, training_filter_split: Optional[str]=None, validation_filter_split: Optional[str]=None, test_filter_split: Optional[str]=None, model_display_name: Optional[str]=None, model_labels: Optional[Dict[str, str]]=None, model_id: Optional[str]=None, parent_model: Optional[str]=None, is_default_version: Optional[bool]=True, model_version_aliases: Optional[Sequence[str]]=None, model_version_description: Optional[str]=None, sync: bool=True, create_request_timeout: Optional[float]=None) -> models.Model:
        """Runs the training job and returns a model.
        ...
        """
        model = gca_model.Model(display_name=model_display_name or self._display_name, labels=model_labels or self._labels, encryption_spec=self._model_encryption_spec)
        return self._run_job(training_task_definition=self._training_task_definition, training_task_inputs=self._training_task_inputs_dict, dataset=dataset, training_fraction_split=training_fraction_split, validation_fraction_split=validation_fraction_split, test_fraction_split=test_fraction_split, training_filter_split=training_filter_split, validation_filter_split=validation_filter_split, test_filter_split=test_filter_split, model=model, model_id=model_id, parent_model=parent_model, is_default_version=is_default_version, model_version_aliases=model_version_aliases, model_version_description=model_version_description, create_request_timeout=create_request_timeout)

    @property
    def _model_upload_fail_string(self) -> str:
        """Helper property for model upload failure."""
        return f'AutoML Text Training Pipeline {self.resource_name} is not configured to upload a Model.'
