...
class _Resource(base.VertexAiResourceNounWithFutureManager, abc.ABC):
    ...
    @codeflash_capture(function_name='_Resource.__init__', tmp_dir_path='/tmp/codeflash_7zxualr6/test_return_values', tests_root='/home/ubuntu/work/repo/tests', is_fto=False)
    def __init__(self, resource_name: Optional[str]=None, resource: Optional[Union[gca_context.Context, gca_artifact.Artifact, gca_execution.Execution]]=None, metadata_store_id: str='default', project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None):
        ...
        if resource is not None:
            ...
            # Optimize dict construction by pre-binding frequently-accessed variables
            parent_resource_name_fields = {metadata.metadata_store._MetadataStore._resource_noun: metadata_store_id}
            ...
            # Avoid repeated lookups by directly storing getattr
            getter_method = getattr(self.api_client, self._getter_method)
            self._gca_resource = getter_method(name=full_resource_name, retry=base._DEFAULT_RETRY)
    ...
    def get_or_create(cls, resource_id: str, schema_title: str, display_name: Optional[str]=None, schema_version: Optional[str]=None, description: Optional[str]=None, metadata: Optional[Dict]=None, metadata_store_id: str='default', project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None) -> '_Resource':
        ...
        """
        # Prevent redundant API calls by immediately returning if ._get finds the resource
        resource = cls._get(resource_name=resource_id, metadata_store_id=metadata_store_id, project=project, location=location, credentials=credentials)
        if resource is None:
    ...
    @classmethod
    def get(cls, resource_id: str, metadata_store_id: str='default', project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None) -> '_Resource':
    ...
    def sync_resource(self):
        ...
        self._gca_resource = getattr(self.api_client, self._getter_method)(name=self.resource_name, retry=base._DEFAULT_RETRY)

    @staticmethod
    def _nested_update_metadata(gca_resource: Union[gca_context.Context, gca_execution.Execution, gca_artifact.Artifact], metadata: Optional[Dict[str, Any]]=None):
    ...
    def update(self, metadata: Optional[Dict]=None, description: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None, location: Optional[str]=None):
        ...
        with self._threading_lock:
            ...
            api_client = self._instantiate_client(credentials=credentials, location=location)
            update_gca_resource = self._update_resource(client=api_client, resource=gca_resource)
            self._gca_resource = update_gca_resource

    @classmethod
    def list(cls, filter: Optional[str]=None, metadata_store_id: str='default', project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None, order_by: Optional[str]=None) -> List['_Resource']:
        ...
        """
        parent = initializer.global_config.common_location_path(project=project, location=location) + f'/metadataStores/{metadata_store_id}'
        return super().list(filter=filter, project=project, location=location, credentials=credentials, parent=parent, order_by=order_by)

    @classmethod
    def _create(cls, resource_id: str, schema_title: str, display_name: Optional[str]=None, schema_version: Optional[str]=None, description: Optional[str]=None, metadata: Optional[Dict]=None, metadata_store_id: Optional[str]='default', project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None) -> Optional['_Resource']:
    ...
    @classmethod
    def _get(cls, resource_name: str, metadata_store_id: Optional[str]='default', project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None) -> Optional['_Resource']:
        ...
        """
        try:
            # Inline call avoids unnecessary variable assignment
            return cls(resource_name, metadata_store_id=metadata_store_id, project=project, location=location, credentials=credentials)
    ...
    def _extract_metadata_store_id(resource_name, resource_noun) -> str:
        ...
        """
        pattern = re.compile('^projects\\/(?P<project>[\\w-]+)\\/locations\\/(?P<location>[\\w-]+)\\/metadataStores\\/(?P<store>[\\w-]+)\\/' + resource_noun + '\\/(?P<id>[\\w-]+)(?P<version>@[\\w-]+)?$')
        match = pattern.match(resource_name)
        if not match:
            raise ValueError(f'failed to extract metadata_store_id from resource {resource_name}')
        return match['store']
