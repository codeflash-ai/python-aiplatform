...
_RESOURCE_STORE_ID_REGEX_CACHE = {}
...
class _Resource(base.VertexAiResourceNounWithFutureManager, abc.ABC):
    ...
    def __init__(
        ...
        resource: Optional[Union[gca_context.Context, gca_artifact.Artifact, gca_execution.Execution]]=None,
        ...
        if resource is not None:
            ...
            # Avoid repeated dict construction for parent_resource_name_fields
            ...
            # get getattr outside to reduce repeated lookups
            getter = getattr(self.api_client, self._getter_method)
            self._gca_resource = getter(
                name=full_resource_name, retry=base._DEFAULT_RETRY
            )
        # Threading lock per-instance for mutability protection
    ...
    def get_or_create(cls, resource_id: str, schema_title: str, display_name: Optional[str]=None, schema_version: Optional[str]=None, description: Optional[str]=None, metadata: Optional[Dict]=None, metadata_store_id: str='default', project: Optional[str]=None, location: Optional[str]=None, credentials: Optional[auth_credentials.Credentials]=None) -> '_Resource':
        ...
        if not resource:
            ...
            resource = cls._create(resource_id=resource_id, schema_title=schema_title, display_name=display_name, schema_version=schema_version, description=description, metadata=metadata, metadata_store_id=metadata_store_id, project=project, location=location, credentials=credentials)
        return resource
    ...
    def _extract_metadata_store_id(resource_name, resource_noun) -> str:
        ...
        """
        # Cache compiled regex patterns by resource_noun for efficiency
        pattern = _RESOURCE_STORE_ID_REGEX_CACHE.get(resource_noun)
        if pattern is None:
            # Only escape resource_noun portion
            escaped_noun = re.escape(resource_noun)
            pattern_str = (
            ...
            _RESOURCE_STORE_ID_REGEX_CACHE[resource_noun] = pattern
