...
def _execute_inference_concurrently(
    ...
    agent_engine: Optional[agent_engines.AgentEngine] = None,
) -> list[Union[genai_types.GenerateContentResponse, dict[str, Any], list[dict[str, Any]]]]:
    ...
    responses: list[
        Union[genai_types.GenerateContentResponse, dict[str, Any], list[dict[str, Any]], None]
    ...
    *,
    ...
    if model:
        ...
        if isinstance(agent_engine, str):
            agent_engine = agent_engines.get(agent_engine)
        ...
        evaluation_dataset = types.EvaluationDataset(
            eval_dataset_df=results_df,
            # TODO(bozhengbz): Add candidate name to agent run results.
...
def _run_agent_internal(
    agent_engine: agent_engines.AgentEngine,
    prompt_dataset: pd.DataFrame
) -> pd.DataFrame:
    """Runs an agent."""
    ...
    if len(processed_responses) != len(prompt_dataset):
        raise RuntimeError(
            "Critical prompt/response count mismatch: %d prompts vs %d"
            " responses. This indicates an issue in response collection."
            % (len(prompt_dataset), len(processed_responses))
        )
...
def _execute_agent_run_with_retry(
    ...
    try:
        user_id = row["agent_engine_inputs"]["user_id"]
        session_state = row["agent_engine_inputs"]["session_state"]
        session = agent_engine.create_session(user_id=user_id, state=session_state)
    except KeyError as e:
        return {"error": f"Failed to get all required agent engine inputs: {e}"}
    except Exception as e:
        return {
            "error": f"Failed to create a new session : {e}"
        }
    for attempt in range(max_retries):
        try:
            responses = []
